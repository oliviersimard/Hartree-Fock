...
mutable struct HubbardStruct
    ...
    matsubara_grid_::Array{Complex{Float64},1}  ## Fermionic Matsubara grid
    matsubara_grid_bosons_::Array{Complex{Float64},1} ## Bosonic Matsubara grid
    ...
    function HubbardStruct(N_iωn_::Int64, dict_::Dict{String,Float64}, N_it_::Int64, beta_::Int64)
        ...
        matsubara_grid_bosons_ = matsubaraGridBosons(N_iωn_,beta_)

        return new(N_iωn_, dict_, N_it_, beta_, matsubara_grid_, matsubara_grid_bosons_)
    end
end
...
function matsubaraGridBosons(N_iωn_::Int64, beta_::Int64)
    N_iωn_ = mod(N_iωn_,2) == 0 ? N_iωn_ : N_iωn_ + 1
    matsubara_grid_ = Array{Complex{Float64},1}(undef,N_iωn_)
    matsubara_grid_ = 1.0im*[(2*n)*pi/beta_ for n in 1:N_iωn_]
...
function BigKArray(funct::Function, Boundaries::Array{Array{Float64,1},1}, GridK::Int64; t::Float64=t_, tp::Float64=tp_, tpp::Float64=tpp_)
    ...
    for (i,ky) in enumerate(range(Boundaries[1][1],stop=Boundaries[2][1],length=GridK))
        for (j,kx) in enumerate(range(Boundaries[2][1],stop=Boundaries[2][2],length=GridK))
            tmpMat[i,j] = funct(kx,ky,t) ## Be careful, because here different dispersion relations would lead to errors.
        end
    end
    return tmpMat
end
